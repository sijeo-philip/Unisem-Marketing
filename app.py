...
def build_recommendation_result(requirement):
    """
    ...
    requirement_summary = (summarize_requirement(requirement))
    ...
    elif (top_candidate.get("status") == "COMPATIBLE"):
        decision = ("recommended")
        recommendation = (top_candidate)
    elif (top_candidate.get("status") == "NEEDS_CLARIFICATION"):
    ...
    # --------------------------------------------------------
    sales_view = (build_sales_view(decision= decision,recommendation=recommendation,top_candidate=top_candidate,
            ranked_results=results, modules=modules))
    return {
        "state": "complete",
        "decision": decision,
 ...
 # ============================================================
# Lesson 2E - Module comparison helpers
# ============================================================


def comparison_display_value(value):
    """
    Convert a raw portfolio value into concise display text.
    """
    if value is None:
        return None
    ...
    if isinstance(value, dict):
        ...
        pieces = []
        for (key,item) in value.items():
            pieces.append(f"{pretty_token(key)}: {item}")
        return "; ".join(pieces)
...
def first_module_value(module,*keys):
    """
    Return the first useful portfolio field from a module.
    """
    if not isinstance(module, dict):
        return None
    for key in keys:
...
def build_comparison_module(module_record, engine_result):
    """
    Create the header information for one module in the
    comparison table.
    """
    module_id = (get_module_identifier(module_record))
    if module_id is None:
        module_id = first_present(engine_result, "module_id", "id", "part_number")

    module_name = first_present(engine_result, "module_name", "name")
    if module_name is None:
        module_name = first_present(module_record, "module_name", "name", "product_name", "marketing_name")
    status = first_present(engine_result, "status")
    choose_when = first_present(engine_result, "choose_when", "recommended_for", "positioning")

    if choose_when is None:
        choose_when = first_present(module_record, "choose_when", "recommended_for", "positioning")
...
def build_comparison_view(selected_module_ids, modules, ranked_results):
    """
    Build a side-by-side comparison model.

    Selection/ranking has already happened in the
    Lesson 1 engine. This function only presents it.
    """
    results_by_id = {}
    for result in ranked_results:
        result_id = first_present(result, "module_id", "id", "part_number", "part_no")
    ...
    for module_id in selected_module_ids:
        module_id = str(module_id)
        module_record = (find_module_record(modules, module_id))
        engine_result = (results_by_id.get(module_id))
        if module_record is None:
            raise ValueError("Unknown module requested for comparison: "+ module_id)
        if engine_result is None:
            raise ValueError(
                "Module was not evaluated by the "
                "selection engine: "
                + module_id
            )
        module_records[module_id] = module_record
        comparison_modules.append(build_comparison_module(module_record, engine_result))
    rows = []

    # --------------------------------------------------------
    # Engine evaluation rows
    # --------------------------------------------------------
    ...
    conflict_values = {}
    for module_info in comparison_modules:
        module_id = (module_info["module_id"])
        result = (results_by_id[module_id])
        status_values[module_id] = (result.get("status", ""))
        passed_values[module_id] = str(len(to_text_list(first_present(result,"passed_requirements",
                        "matched_requirements","requirements_satisfied"))))
        clarification_values[module_id] = str(len(to_text_list(first_present(result, "clarifications",
                        "clarification_items"))))
        conflict_values[module_id] = str(len(to_text_list(first_present(result,"hard_failures","conflicts",
                        "failed_requirements"))))
    rows.append(
        {
            "key": "fit_status",
    ...
    # --------------------------------------------------------
    # Portfolio specification rows
    # --------------------------------------------------------

    for (field_key, field_label, possible_keys) in COMPARISON_FIELDS:
        values = {}
        any_value_found = False
        for module_info in comparison_modules:
            module_id = (module_info["module_id"])
            module_record = (module_records[module_id])
            raw_value = (first_module_value(module_record,*possible_keys))
            display_value = (comparison_display_value(raw_value))
            if display_value is not None:
                any_value_found = True
            values[module_id] = (display_value or "—")
        # Do not create empty specification rows.
...
def requirement_from_answers(answers):
    """
    Replay browser answers through QuestionSession and
    ...
    return session.get_requirement()
# ============================================================
...
def api_compare_modules():
    """
    Compare one or more evaluated modules against the
    completed customer requirement.

    One module is allowed so the current single-module
    portfolio can use the same UI as a detailed
    specification view.

    Up to four modules may be displayed side-by-side.
    """
    ...
    try:
        ...
        # ----------------------------------------------------
        # Reconstruct authoritative requirement
        # ----------------------------------------------------
        requirement = (requirement_from_answers(answers))
        # ----------------------------------------------------
        # Re-run selection engine
        # ----------------------------------------------------
        ...
        print("Unexpected comparison error:", repr(exc))
        return jsonify(
            {
                "status": "error",
                "error": ("The module comparison "
